...
function bind(hanbotz) {
    ...
    hanbotz.ev.on('connection.update', update => {
        Object.assign(state, update)
    })

    hanbotz.ev.on('chats.set', ({ chats: newChats, isLatest }) => {
        /**if (isLatest) {
            chats.clear()
        }**/
    ...
    hanbotz.ev.on('contacts.set', ({ contacts: newContacts }) => {
    ...
    hanbotz.ev.on('messages.set', ({ messages: newMessages, isLatest }) => {
        if (isLatest) {
   ...
   hanbotz.ev.on('contacts.update', updates => {
        for (const update of updates) {
            Object.assign(contacts[update.id], update)
        }
    })

    hanbotz.ev.on('chats.upsert', newChats => {
        chats.upsert(...newChats)
    })

    hanbotz.ev.on('messages.delete', item => {
        if ('all' in item) {
    ...
    hanbotz.ev.on('chats.delete', deletions => {
        for (const item of deletions) {
            chats.deleteById(item)
        }
    })

    hanbotz.ev.on('chats.update', updates => {
    ...
    hanbotz.ev.on('presence.update', ({ id, presences: update }) => {
        presences[id] = presences[id] || {}
        Object.assign(presences[id], update)
    })

    hanbotz.ev.on('messages.upsert', ({ messages: newMessages, type }) => {
        switch (type) {
            case 'append':
            case 'notify':
                for (const msg of newMessages) {
    ...
    /**hanbotz.ev.on('messages.update', updates => {
    ...
    hanbotz.ev.on('groups.update', updates => {
        for (const update of updates) {
    ...
    hanbotz.ev.on('message-receipt.update', updates => {
    ...
    return {
        ...
        loadMessages: async (jid, count, cursor) => {
            ...
            const retrieve = async (count, cursor) => {
                const result = await (hanbotz === null || hanbotz === void 0 ? void 0 : hanbotz.fetchMessagesFromWA(jid, count, cursor))
                return result || []
            }
        ...
        loadMessage: async (jid, id, hanbotz) => {
            var _a
            let message = (_a = messages[jid]) === null || _a === void 0 ? void 0 : _a.get(id)
            if (!message) {
                message = await (hanbotz === null || hanbotz === void 0 ? void 0 : hanbotz.loadMessageFromWA(jid, id))
            }
            return message
        },
        mostRecentMessage: async (jid) => {
            ...
            if (!message) {
                const [result] = await (hanbotz === null || hanbotz === void 0 ? void 0 : hanbotz.fetchMessagesFromWA(jid, 1, undefined))
                message = result
            }
            return message
        },
        fetchImageUrl: async (jid) => {
            ...
            if (!contact) {
                return hanbotz === null || hanbotz === void 0 ? void 0 : hanbotz.profilePictureUrl(jid)
            }
            if (typeof contact.imgUrl === 'undefined') {
                contact.imgUrl = await (hanbotz === null || hanbotz === void 0 ? void 0 : hanbotz.profilePictureUrl(jid))
            }
            return contact.imgUrl
        },
        fetchGroupMetadata: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (hanbotz === null || hanbotz === void 0 ? void 0 : hanbotz.groupMetadata(jid))
            }
            return groupMetadata[jid]
        },
        fetchBroadcastListInfo: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (hanbotz === null || hanbotz === void 0 ? void 0 : hanbotz.getBroadcastListInfo(jid))
            }
            return groupMetadata[jid]
        },
        fetchMessageReceipts: async ({ remoteJid, id }) => {
            ...
            if (!receipts) {
                receipts = await (hanbotz === null || hanbotz === void 0 ? void 0 : hanbotz.messageInfo(remoteJid, id))
                if (msg) {
